...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  useEffect(() => {
    const updateMobileMode = () => setIsMobileTooltipMode(window.innerWidth < 640);
    updateMobileMode();
  ...
  const getWhatsAppUrl = () => {
    ...
    const shareText = `I'm interested in the following item from ${store?.storeName} — ${product.name} — ₦${parseFloat(product.price).toLocaleString()}`;
  ...
  const handleWhatsAppConfirm = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <>
            ...
            <Tooltip open={isMobileTooltipMode ? isFocused : undefined}>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-[#25D366]/10 hover:bg-[#25D366] text-[#25D366] hover:text-white flex items-center justify-center transition-transform duration-200 ease-out hover:-translate-y-1 hover:scale-105 motion-safe:hover:animate-bounce active:scale-95 shrink-0"
              ...
              <TooltipContent className="bg-gray-900 text-white text-xs rounded-md p-2">
      ...
      <Dialog open={isWhatsAppModalOpen} onOpenChange={setWhatsAppModalOpen}>
        <DialogContent className="max-w-md rounded-[24px] bg-white p-6">
          <DialogHeader>
            <DialogTitle>Send WhatsApp message?</DialogTitle>
            <DialogDescription>
              Confirm before opening WhatsApp for {product.name} from {store?.storeName}.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="mt-6 gap-3">
            <Button variant="outline" type="button" onClick={() => setWhatsAppModalOpen(false)}>
              Cancel
            </Button>
            <Button type="button" onClick={handleWhatsAppConfirm}>
              Confirm
            </Button>
          </DialogFooter>
        </DialogContent>
